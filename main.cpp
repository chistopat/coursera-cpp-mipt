...
public:
    ...
    tuple<TasksInfo, TasksInfo> PerformPersonTasks(const string& person, int task_count) {
        if (tracker.count(person) == 1) {
            auto& currentTasks = tracker.at(person);
            TasksInfo updated;
            auto untouched = currentTasks;
            for(auto& [status, value] : currentTasks) {
                if (status != TaskStatus::DONE) {
                    auto nextStatus = TaskStatus(static_cast<int>(status) + 1);
                    int newValue = min(task_count, value);
                    currentTasks[status] -= newValue;
                    currentTasks[nextStatus] += newValue;
                    untouched[status] -= newValue;
                    untouched[nextStatus] = currentTasks[nextStatus] - newValue;
                    updated[nextStatus] += newValue;
                    task_count -= newValue;
                }
            }
            untouched.erase(TaskStatus::DONE);
            for (auto& [pers, tasks] : tracker) {
                tracker[pers] = TaskInfoErase(tasks);
            }
            return make_tuple(TaskInfoErase(updated), TaskInfoErase(untouched));
        }
        return make_tuple(TasksInfo(), TasksInfo());
    }
private:
    map<string, TasksInfo> tracker;
    static TasksInfo TaskInfoErase(TasksInfo& tasksInfo) {
        TasksInfo result;
        for (auto& [k,v] : tasksInfo) {
            if(v > 0) {
                result[k] = v;
            }
        }
    ...
    static TasksInfo& IncrementTaskInfo(TasksInfo& tasksInfo) {
        auto& [minStatus, minValue] = *tasksInfo.begin();
        if(minStatus!=TaskStatus::DONE) {
            auto nextStatus = TaskStatus(static_cast<int>(minStatus)+1);
            tasksInfo[minStatus]--;
            tasksInfo[nextStatus]++;
            if(tasksInfo[minStatus]==0) {
                tasksInfo.erase(minStatus);
            }
        }
        return tasksInfo;
    }
