...
public:
  ...
  [[nodiscard]] size_t Get(TAirport airport) const {
    return data_[GetIndex(airport)];
  }

  // добавить данный элемент
  void Insert(TAirport airport) {
    ++data_[GetIndex(airport)];
  }
  ...
  void EraseAll(TAirport airport) {
    data_[GetIndex(airport)] = 0;
  }

public:
  static constexpr uint32_t kAirportsCount
      = static_cast<uint32_t>(TAirport::Last_);
public:
  using Item = pair<TAirport, size_t>;
  using Items = array<Item, kAirportsCount>;
  ...
  [[nodiscard]] Items GetItems() const {
    Items result;
    for (uint32_t i = 0; i < kAirportsCount; ++i) {
      result[i] = {GetAirport(i), data_[i]};
    }
    return result;
  }

private:
  array<uint32_t, kAirportsCount> data_;
private:
  static uint32_t GetIndex(TAirport airport) {
    return static_cast<uint32_t>(airport);
  }
  static TAirport GetAirport(uint32_t index) {
    return static_cast<TAirport>(index);
  }
};
