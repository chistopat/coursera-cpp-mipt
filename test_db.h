...
struct Entry {
    Entry(vector<int> d, std::string e) : date(d), event(e) {}
    vector<int> date;
    std::string event;
};

bool operator!=(Entry e, DbEntry dbe) {
    return e.date != dbe.date and e.event != dbe.event;
}

bool operator!=(DbEntry dbe, Entry e) {
    return e.date != dbe.date and dbe.event != e.event;
}

std::ostream& operator<<(std::ostream& os, Entry e) {
    os << Date(e.date[0], e.date[1], e.date[2]);
    os << " " << e.event;
    return os;
}
