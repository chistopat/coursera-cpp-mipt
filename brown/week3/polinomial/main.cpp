...
public:
    class IndexProxy {
    public:
        IndexProxy(Polynomial& polynomial, size_t degree)
        ...
        operator T() const {
            return as_const(polynomial_)[degree_];
        }

        IndexProxy& operator=(T coefficient) {
            if (degree_ >= polynomial_.coeffs_.size()) {
                polynomial_.coeffs_.resize(degree_+1);
            }
            polynomial_.coeffs_[degree_] = coefficient;
            polynomial_.Shrink();
            return *this;
        }
